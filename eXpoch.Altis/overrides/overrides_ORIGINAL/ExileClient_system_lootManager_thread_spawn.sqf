...
_buildings = nearestObjects [player,["House"], _spawnRadius];
_newTypes = getArray(missionConfigFile >> "CfgeXpochLooting" >> "NewLootBuildingTypes");
{
	_buildings append (nearestObjects [player,[_x], _spawnRadius]);
}forEach _newTypes;
