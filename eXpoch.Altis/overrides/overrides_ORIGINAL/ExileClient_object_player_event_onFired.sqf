...
private["_unit","_weapon","_muzzle","_mode","_ammo","_magazine","_projectile","_object","_intersection","_firstInsection","_swings"];
...
else 
{
	switch (_weapon) do 
	{
		...
		case "MeleeSledge":
		{
			...
			[] call ExileClient_object_shippingContainer_smash;
			[] call eXpochClient_object_player_mining_smashRock;
		};
		case "MeleeRod":
		{
			player playActionNow "SledgeSwing";
			[] call eXpochClient_object_player_fishing_preCast;
		};
		case "Hatchet":
		{
			_swings = ["GestureSwing0","GestureSwing1","GestureSwing2"]; 
			player playActionNow selectRandom(_swings);
			[] call ExileClient_object_tree_chop;
		};
		case "CrudeHatchet":
		{
