...
private["_sessionID","_requestingPlayer","_spawnLocationMarkerName","_bambiPlayer","_accountData","_direction","_position","_spawnAreaPosition","_spawnAreaRadius","_clanID","_clanData","_clanGroup","_player","_devFriendlyMode","_devs","_parachuteNetID","_spawnType","_parachuteObject","_respect","_rankMult","_adjRespect","_rank"];
...
//THIS IS A FREE VARIABLE TO USE FOR ACE from createPlayerRequest :) _bambiPlayer = _this select 3;
_loadAceStats = _this select 3;
...
_direction = 270; //changed for customization screen//random 360;
...
diag_log format["eXpoch_Debug: createBambi - _spawnLocationMarkerNameArray:%1",_spawnLocationMarkerNameArray];
/*if!(isNil "_spawnType")then
{
	_spawnType = _spawnLocationMarkerNameArray select 2;
...
if (_spawnType isEqualTo 1) then
{
