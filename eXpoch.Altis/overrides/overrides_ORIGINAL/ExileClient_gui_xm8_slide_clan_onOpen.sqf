...
_clanMemberListBox lbSetCurSel -1;
{
	...
	_isModerator = (_x select 0) in (ExileClientClanInfo select 6);
	if(_isLeader)then
	{
		_clanMemberListBox lbSetPicture [_index, "\a3\ui_f\data\gui\cfg\Ranks\colonel_gs.paa"];
	}
	else 
	{
		if(_isModerator)then
		{
			_clanMemberListBox lbSetPicture [_index, "\a3\ui_f\data\gui\cfg\Ranks\captain_gs.paa"];
		}else{
