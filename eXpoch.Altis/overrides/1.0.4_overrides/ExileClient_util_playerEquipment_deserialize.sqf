 ...
 * use of addWeaponGlobal according to BIS Wiki is broken on dedicated servers
 */
...
if !((_equipment select 4) isEqualTo "") then { _player addWeapon 	(_equipment select 4); }; 
...
{ 
	_player addWeapon (_equipment select 7);
	{ 
		if !(_x isEqualTo "") then 
		{	
...
{ 
	_player addWeapon (_equipment select 10);
	{ 
		if !(_x isEqualTo "") then 
		{	
...
{ 
	_player addWeapon (_equipment select 13);
	{ 
		if !(_x isEqualTo "") then 
		{	
