...
if (diag_tickTime - ExileHudStatsRenderedAt >= 0.25) then
{
	...
	if(isNil "ExileClientPlayerUSATemp")then
	{
		ExileClientPlayerUSATemp = 1;
	};
	/* eXpoch New Addition */
	if(ExileClientPlayerUSATemp isEqualTo 0)then
	{
	...
	else
	{
		...
		_environmentTemperature = [ExileClientEnvironmentTemperature, 1] call ExileClient_util_math_round;
		_FenvironmentTemperature = _environmentTemperature * 1.8 + 32;
		_FenvironmentTemperatureRound = round(_FenvironmentTemperature);
		_environmentTemperatureValueControl = _display displayCtrl 1308;
		_environmentTemperatureValueControl ctrlSetText format ["%1%2", _FenvironmentTemperatureRound, "°F"];
	};
};