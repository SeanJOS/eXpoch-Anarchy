/**
 * ExileClient_gui_vehicleTraderDialog_updateVehicleListBox_blackmarket
 *
 * Exile Mod
 * www.exilemod.com
 * © 2015 Exile Mod Team
...
private["_dialog","_categoryClasses","_itemListControl","_categoryClass","_categoryVehicleClassNames","_className","_salesPrice","_indexEntryIndex","_playerMoney","_quality","_requiredRespect","_qualityColor","_popTabColor","_missingPopTabs","_missingRespect","_playereXpochMoney","_availableItem","_rankAllowed","_configHeroBanditRank"];
...
lbClear _itemListControl;
{
	...
	_categoryVehicleClassNames = getArray(missionConfigFile >> "CfgTraderCategories" >> _categoryClass >> "items");
	{
		...
		_rankAllowed = getNumber(missionConfigFile >> "CfgExileArsenal" >> _className >> "traderRank");
		_configHeroBanditRank = getNumber(missionConfigFile >> "CfgeXpochRank" >> "heroNbanditRank");
		if(((eXpochClientPlayerRank >= _rankAllowed) || (eXpochClientPlayerRank <= -(_rankAllowed))) && (_rankAllowed < _configHeroBanditRank))then
		{
			_availableItem = true;
		};
