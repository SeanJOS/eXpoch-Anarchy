...
while {true} do
{
	...
	while {true} do
	{
		...
		while {true} do
		{
			...
			_players = [];
			_players = nearestObjects[position _zombie,["Exile_Unit_Player","eXpoch_Female_Prisoner_F"],400];
			if(_players isEqualTo [])then
			{
				breakTo "findtarget";
			};
			_target = _players select 0;
	// add check for agents. Animal spawns and best friends
			...
			if !(isnull _target) then
			{
				...
				{
					if ((_sound == 0) && (isnil "ryanzombiesdisableaggressive")) then {_Aggressive = selectRandom _AggressiveArray; playsound3d [format ["%1",_Aggressive], _zombie, false, getPosASL _zombie, 0.7, _soundpitch];};
					_sound = _sound + 1;
				...
				if (!(alive _target) && !(isnil "Ryanzombiesfeed") && (vehicle _target iskindof "man")) then
				{
					...
					while {(_zombie distance _target < 80) && (alive _zombie)} do
					{
						...
						if ((_zombie distance _target < 3) && (_c < _d)) then
						{
							...
							{
								if (random 2 < 1) then {_Eating = selectRandom _EatingArray; playsound3d [format ["%1",_Eating], _zombie, false, getPosASL _zombie, 0.7, _soundpitch];};
